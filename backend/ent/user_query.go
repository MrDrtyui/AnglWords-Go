...
func (_q *UserQuery) QueryWords() *WordQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, user.WordsTable, user.WordsPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *UserQuery) WithWords(opts ...func(*WordQuery)) *UserQuery {
	query := (&WordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) loadWords(ctx context.Context, query *WordQuery, nodes []*User, init func(*User), assign func(*User, *Word)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.WordsTable)
		s.Join(joinT).On(s.C(word.FieldID), joinT.C(user.WordsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.WordsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.WordsPrimaryKey[0]))
