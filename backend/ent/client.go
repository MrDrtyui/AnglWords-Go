...
func (c *UserClient) QueryWords(_m *User) *WordQuery {
...
func (c *WordClient) QueryUser(_m *Word) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		RefreshToken, User, Word []ent.Hook
	}
	inters struct {
		RefreshToken, User, Word []ent.Interceptor
	}
)
